...
void LCD_ShowPic(int x1, int y1, int w, int h, u8* img, u8 flipped) {
  int x1_real = x1 < 0 ? 0 : x1;
  int x2_real = x1 + w > WIDTH ? WIDTH - 1 : x1 + w - 1;
  int y1_real = y1 < 0 ? 0 : y1;
  int y2_real = y1 + h > HEIGHT ? HEIGHT - 1 : y1 + h - 1;
  LCD_Address_Set(x1_real, y1_real, x2_real, y2_real);
  OLED_DC_Set();
  for(int i = y1_real; i <= y2_real; ++i){
    for(int j = x1_real; j <= x2_real; ++j){
      u8 cur1 = img[((j - x1) + w * (i - y1)) << 1];
      u8 cur2 = img[((j - x1) + w * (i - y1)) << 1 | 1];
      LCD_Writ_Bus(flipped ? (0xff - cur1) : cur1);
      LCD_Writ_Bus(flipped ? (0xff - cur2) : cur2);
    }
  }
}
